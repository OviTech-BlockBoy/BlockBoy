...
static void system_monitor_task(void *arg)
{
    ...
    while (!exitCalled)
    {
        ...
        // Display mode timer - auto-exit after 60 seconds
        ...
        if (!app.isLauncher && rg_settings_get_number(NS_GLOBAL, "DisplayActive", 0))
        {
            ...
            if (display_timer >= 60)
            {
                RG_LOGI("Display mode: switching to next game");
                rg_system_switch_app(RG_APP_LAUNCHER, 0, 0, 0);
            }
            // Check for MENU press to cancel display mode
...
void rg_system_exit(void)
{
    ...
    // If display mode is active and user is quitting, deactivate it
    if (!app.isLauncher && rg_settings_get_number(NS_GLOBAL, "DisplayActive", 0))
    {
        rg_settings_set_number(NS_GLOBAL, "DisplayActive", 0);
        rg_settings_commit();
