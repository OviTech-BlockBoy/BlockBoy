...
// Game Link: frame types for the serial connection. Must equal RG_LINK_* in
// rg_netplay.h (main_gbc.c guards that with a _Static_assert).
// The master clocks a byte out with a sequence number; the partner replies
// with that same number. This makes a late reply to an aborted transfer
// recognizable, so no byte ever needs to be thrown away - which used to break
// every game that exchanges several bytes in a row.
#define GB_LINK_MASTER 1   /* master clocks a byte out                     */
#define GB_LINK_REPLY  2   /* reply from the partner, with the same seq    */

typedef void (gb_serial_send_cb_t)(uint8_t type, uint8_t seq, uint8_t data);
typedef bool (gb_serial_recv_cb_t)(uint8_t *type, uint8_t *seq, uint8_t *data);
