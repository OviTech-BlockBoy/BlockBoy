...
void gnuboy_run(bool draw)
{
	...
	// We emulate until vblank (0..144). Bail out early if the game disables
	// the LCD mid-frame — when LCD is off, gb_lcd_emulate does not advance R_LY,
	// so this loop would otherwise spin forever.
	while (R_LY <= 144 && (R_LCDC & 0x80)) {
		cycles += 228;
		cycles -= gb_cpu_emulate(cycles);
	}

	// If the LCD got disabled before vblank, skip the rest of the frame.
	if (!(R_LCDC & 0x80))
		return;
	...
	// Emulate vblank (145...0). Same LCD-on guard as the main loop above.
	while (R_LY > 0 && (R_LCDC & 0x80)) {
		cycles += 228;
		cycles -= gb_cpu_emulate(cycles);
	}
