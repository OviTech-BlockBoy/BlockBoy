...
enum
{
    ...
    RG_TEXT_NO_SYNC      = (1 << 8),   // direct LCD write without display-task sync.
                                       // REQUIRED when drawing from the display task itself
                                       // (sync would wait on itself = deadlock).
};
...
// Keyboard layout for the virtual keyboard / rg_gui_input_str
typedef struct
{
    const char *layout;   // UTF-8 string of all keys, row by row
    size_t columns;
    size_t rows;
    const char *label;    // hint shown in the bottom bar (e.g. "ABC", "abc", "!@#")
...
/* -- Single registry of menu entries, each tagged with a tab id.                -- */
/* -- Per-entry runtime visibility via NS_GLOBAL setting "Show_<vis_key>".       -- */
/* -------------------------------------------------------------------------------- */

typedef struct
{
    int tab_id;                  // which tab this entry belongs to (0..num_tabs-1); -1 = end of list
    const char *vis_key;         // setting key for runtime hide/show; NULL = always visible
    bool default_vis;            // default visibility if no setting stored yet
    rg_gui_option_t option;      // the standard option (label, value, flags, callback)
} rg_gui_menu_entry_t;

#define RG_MENU_ENTRY_END    {-1, NULL, false, RG_DIALOG_END}
...
// PIN-protected dev menu. PIN sequence: UP UP DOWN A.
// Returns true on correct entry, false on cancel/wrong attempt.
bool rg_gui_verify_pin(void);

// Shop Menu -- lets the operator toggle which menu entries are visible.
// Modifies NS_GLOBAL "Show_<vis_key>" settings; changes take effect on next menu open.
...
void rg_gui_draw_loading(void); // This should be moved to system or display...
...
// Weak hooks, overridden by the launcher: start the HTTP file server for this
// session (weak default returns false, i.e. emulators), and report the password
// it asks for (weak default returns NULL = no password).
bool rg_gui_file_server_start(void);
const char *rg_gui_file_server_password(void);
bool rg_gui_file_server_running(void);
// Lets the user change the file server password. True = changed.
// The weak default does nothing, which hides the row in the emulators.
bool rg_gui_file_server_set_password(void);
// Turns the file server on or off and remembers it. True = succeeded.
...
void rg_gui_set_showcase_game_name(const char *name);
