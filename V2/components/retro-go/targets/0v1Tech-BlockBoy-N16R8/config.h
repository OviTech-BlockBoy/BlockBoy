...
#define RG_PROJECT_VER             "V2-2.1.0"
...
// share that name while being different devices.
#define RG_OTA_MODEL "blockboy-v2-n16r8"

// Semver, only used for comparing versions. Bump on every release.
#define RG_FIRMWARE_VERSION "2.1.0"

// Bump whenever the partition layout in rg_config.py changes.
#define RG_LAYOUT_VERSION 1

#define RG_OTA_MANIFEST_URL \
    "https://raw.githubusercontent.com/OviTech-BlockBoy/BlockBoy/main/ota/v2-n16r8.json"

// Storage
#define RG_STORAGE_ROOT             "/sd"
#define RG_STORAGE_SDSPI_HOST       SPI3_HOST
#define RG_STORAGE_SDSPI_SPEED      SDMMC_FREQ_DEFAULT  // 20MHz (most stable). 40MHz tested = no gain (avg_read 54ms unchanged = latency-bound, not bandwidth) and unstable (6s stalls) -> reverted.
...
#define RG_SCREEN_SPEED             SPI_MASTER_FREQ_40M // 80M garbled: LCD-CLK=GPIO48 goes through the GPIO matrix (cap 40M); no intermediate step possible (S3 SPI = 80/n)
#define RG_SCREEN_BACKLIGHT         1
#define RG_SCREEN_WIDTH             320
#define RG_SCREEN_HEIGHT            240
#define RG_SCREEN_ROTATE            0
#define RG_SCREEN_VISIBLE_AREA      {0, 0, 37, 6} // Left, Top, Right, Bottom
...
// Rotation bits only -- BGR is applied live via the "Color order" menu
#define RG_SCREEN_MADCTL_ROT (ILI9341_MADCTL_MY | ILI9341_MADCTL_MV)
...
#define RG_SCREEN_INIT()                                                                                         \
    ...
    ILI9341_CMD(ILI9341_MADCTL, RG_SCREEN_MADCTL_ROT);  /* Color order (BGR bit) applied later, see rg_display_set_color_order() */ \
...
// Resistor-divider ratio. This is the THEORETICAL value; it has not been checked
// against a multimeter on this board revision. On the V3 board the theoretical 2.f
// turned out to be ~10% off (4.10V real vs 3.72V computed). Measure per revision and
// correct here; the "Batt. calibration" menu trims it at runtime in the meantime.
// This matters more here than on V3: without a status LED the on-screen battery icon
// and the Batt. warning voltage threshold are the ONLY low-battery signals.
#define RG_BATTERY_VDIV_RATIO        2.f
...
// Deep-sleep wake (showcase auto on/off): pressing any of these buttons wakes
// the device. Only GPIO0-21 are RTC-capable on the ESP32-S3, so START(46)/
// B(45)/LEFT/RIGHT can't be used. A/SELECT/UP/DOWN cover it.
#define RG_GPIO_WAKE_MASK  ((1ULL << GPIO_NUM_7) | (1ULL << GPIO_NUM_8) | \
