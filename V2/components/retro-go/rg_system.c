...
static float cached_battery_level = -1; // last % read by the tick loop, -1 = no battery
static rg_app_t app;
...
static void system_monitor_task(void *arg)
{
    ...
    while (!exitCalled)
    {
        ...
        cached_battery_level = battery.present ? battery.level : -1;
        ...
        // Auto frameskip (only when running at normal speed). frameskip < 0 = "Off" → respect it
        // and never auto-raise (otherwise "Off" silently climbs to 5 during heavy scenes).
        if (app.speed == 1.f && app.frameskip >= 0 && statistics.ticks > app.tickRate * 2)
        {
...
rg_app_t *rg_system_init(int sampleRate, const rg_handlers_t *handlers, void *_unused)
{
    ...
    if (app.isColdBoot && bootanim_mode != 2)  // Only on cold boot and if not disabled
    {
        ...
        rg_display_draw_led_dot(); // keep the fixed LED visible during the boot screen
        rg_task_delay(200);
        ...
        rg_display_draw_led_dot(); // on top of the held logo
...
{
    // Time only ever moves forward, so start from the freshest source we have:
    //  - rtcValue: kept in RTC memory, survives soft resets (the showcase
    //    game-switches reboot the device every "Switch time" seconds), so it is
    //    normally the most up-to-date.
    //  - RG_BUILD_TIME: a floor so we never start before the firmware existed.
    time_t time_sec = RG_MAX(rtcValue, RG_BUILD_TIME);
#ifdef ESP_PLATFORM
    {
        // The SoC keeps its RTC timer running across a soft reset; if it already
        // restored a newer time, honour it.
...
bool rg_system_sync_time_now(void)
{
    ...
    int64_t deadline = rg_system_timer() + 25 * 1000000; // generous: runs in the background
    ...
    int64_t sntp_deadline = rg_system_timer() + 30 * 1000000; // first NTP sync can be slow
    bool ok = false;
...
float rg_system_get_battery_level(void)
{
...
#endif
#ifdef RG_GPIO_LED
    gpio_reset_pin(RG_GPIO_LED);
    gpio_set_direction(RG_GPIO_LED, GPIO_MODE_OUTPUT);
    gpio_set_level(RG_GPIO_LED, 0);        // active-high LED: LOW = off
