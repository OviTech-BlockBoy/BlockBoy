...
/* Render-to-core0 offload (dual-core). Master switch over the per-core render
 * context, VBlank snapshot and core0 render dispatch. 0 = single-core behaviour. */
#define RENDER_OFFLOAD 1
/* PSRAM margin left free for the offload snapshot + savestate buffer + slack. */
#define RENDER_OFFLOAD_PSRAM_RESERVE  (560 * 1024)
extern void render_ctx_bind_live(void);    /* video.cpp; bind this core's ctx to the live globals */
extern int  render_offload_init(void);     /* video.cpp; allocate the snapshot buffer (once) */
extern void render_ctx_bind_snap(u16 *out);/* video.cpp; bind this core's ctx to the snapshot */
extern void render_snap_capture(void);     /* video.cpp; live render state -> g_snap (emu core @VBlank) */
extern void render_offload_frame(u16 *out);/* video.cpp; render g_snap -> out (on the render core) */
#if RENDER_OFFLOAD
extern u32 g_predict_clean;     /* 1 = next frame predicted clean -> skip render-core work */
extern u32 g_offload_active;    /* 1 = offload on (1.0x, no frameskip); 0 = live on core0 */
#endif
