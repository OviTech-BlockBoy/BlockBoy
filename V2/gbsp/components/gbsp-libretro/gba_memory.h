...
/* Savestate-reserve trick: temporarily release the partial ROM cache block so the big
 * transient savestate buffer fits (release -> alloc/save/free -> restore). Only call
 * with emulation paused. release returns the freed bytes (0 = no partial block);
 * restore is a no-op when there is nothing to put back. */
u32  gamepak_cache_release_partial(void);
void gamepak_cache_restore_partial(void);

/* Bulk write for the LZ77 HLE: memcpy to EWRAM/IWRAM (and VRAM when vram_ok, the
 * 16-bit variant) with side effects identical to write_memory*. 0 = not handled →
 * caller falls back to the per-unit write_memory loop. */
...
typedef struct
{
  ...
  // From video.cpp, obj_priority_list lives behind gbsp_obj_priority_list below.
  // Still in static BSS (small and hot): obj_priority_count, obj_alpha_count.
} gbsp_memory_t;
...
// iwram and vram are the hottest GBA memories (every interpreted CPU access and
// all PPU rendering goes through them). They are allocated separately so they can
// live in internal SRAM instead of PSRAM (~10x lower latency). The pointer-to-array
// form keeps sizeof(), indexing and memset() semantics identical to a real array.
extern u8 (*gbsp_vram)[1024 * 96];
extern u8 (*gbsp_iwram)[(1024 * 32) << SMC_DETECTION];
#define vram (*gbsp_vram)
#define iwram (*gbsp_iwram)

// PPU sprite bookkeeping (100KB), rebuilt every rendered frame and only read
// back per scanline — tolerates PSRAM latency. Allocated with MEM_SLOW in
// main.c so vram can take its place in internal SRAM. Defined in video.cpp.
extern u8 (*gbsp_obj_priority_list)[5][160][128];
#endif
