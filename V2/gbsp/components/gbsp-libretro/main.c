...
#if RENDER_OFFLOAD
  // Bind this (emu) core's render context to the live globals for the
  // per-scanline render this frame. .out follows the buffer swap (currentUpdate->data).
  render_ctx_bind_live();
#endif
...
#if RENDER_OFFLOAD
          // Skip the core0 render only when offload is active (1.0x, no frameskip)
          // and the frame is predicted clean → the render core draws it from the
          // snapshot. Otherwise render live per-scanline on core0.
...
#if RENDER_OFFLOAD
              // HBlank DMA during HDraw is a per-scanline raster effect that does not
              // go through write_memory → mark as mid-frame dirty (bit 16). Required for
              // RENDER_OFFLOAD: otherwise battle waves get misdetected as clean.
              extern u32 g_m2_dirty; g_m2_dirty |= 16;
#endif
            }
          }
        }
...
{
#if RENDER_OFFLOAD
  // Bind the render context before init/resume: init_main + savestate load call
  // video_reload_counters (writes affine via the ctx) before the first update_gba;
  // without this bind = NULL deref. gbsp_vram/gba_screen_pixels are already valid here.
  render_ctx_bind_live();
#endif
  init_memory();
  init_main();
