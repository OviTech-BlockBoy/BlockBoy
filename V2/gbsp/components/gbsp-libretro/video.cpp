...
#if RENDER_OFFLOAD
/* ===== Per-core render context ==========================================
 * With render-to-core0 offload, core0 reads from a snapshot while core1 emulates
 * the next frame via the file-scope globals. To avoid the data race there is one
 * render_ctx per core (indexed by core id). All fields are pointers: the live ctx
 * points at the real globals (no behaviour change), the offload ctx at the
 * snapshot. The render-internal scratch is shared and lock-protected on transitions. */
#undef vram          /* gba_memory.h's '#define vram (*gbsp_vram)' must go before the
                        'vram' struct field (else it expands inside the declaration). */
typedef struct {
  u8  (*vram)[1024 * 96];
  u16 *pal;
  u16 *oam;
...
#ifdef ESP_PLATFORM
#include "esp_cpu.h"
#define R_CTX  (g_rctx[esp_cpu_get_core_id()])   /* core-local render context */
#else
#define R_CTX  (g_rctx[0])
#endif

/* Bind a ctx to the live emulation globals. MUST come before the macro redirects so
 * it sees the real symbols (not the rc-> versions). Called per frame from update_gba:
 * the pointers are stable, only .out changes with the buffer swap. */
extern "C" void render_ctx_bind_live(void)
{
  /* Bind the current (emu) core to the live globals. Fields are pointers to the same
   * global storage → byte-identical to the original renderer. The render task (the
   * other core) binds its own core to the snapshot (render_ctx_bind_snap) → no
   * conflict, different core id. Called early in reset_gba to cover the resume
   * render (savestate video_reload_counters) before the first update_gba. */
#ifdef ESP_PLATFORM
  int core = esp_cpu_get_core_id();
#else
  int core = 0;
#endif
  g_rctx[core].vram = gbsp_vram;
  g_rctx[core].pal  = palette_ram_converted;
  g_rctx[core].oam  = oam_ram;
  g_rctx[core].io   = io_registers;
  g_rctx[core].affx = affine_reference_x;
  g_rctx[core].affy = affine_reference_y;
  g_rctx[core].out  = gba_screen_pixels;
}

/* Snapshot buffer: at VBlank core1 copies the render state here (vram 96KB +
 * pal + oam + io) for a clean frame; core0 renders from it. Single buffer (~99KB
 * PSRAM); a sync flag covers the rare overlap case. */
#ifdef ESP_PLATFORM
#include "esp_heap_caps.h"
#endif
#define SNAP_VRAM_BYTES   (1024 * 96)
#define SNAP_TOTAL_BYTES  (SNAP_VRAM_BYTES + 4096)   /* vram + pal/oam/io in the tail */
u8  *g_snap;
s32  g_snap_affx[2], g_snap_affy[2];

/* Bind the current core to the snapshot: core1 filled g_snap at VBlank, core0
 * renders from it into 'out'. Defined before the macro redirects so the struct
 * fields (vram/pal/...) are fields here, not macros. */
extern "C" void render_ctx_bind_snap(u16 *out)
{
#ifdef ESP_PLATFORM
  int core = esp_cpu_get_core_id();
#else
  int core = 0;
#endif
  g_rctx[core].vram = (u8 (*)[SNAP_VRAM_BYTES])g_snap;
  g_rctx[core].pal  = (u16 *)(g_snap + SNAP_VRAM_BYTES);
  g_rctx[core].oam  = (u16 *)(g_snap + SNAP_VRAM_BYTES + 1024);
  g_rctx[core].io   = (u16 *)(g_snap + SNAP_VRAM_BYTES + 2048);
  g_rctx[core].affx = g_snap_affx;
  g_rctx[core].affy = g_snap_affy;
  g_rctx[core].out  = out;
}

/* Copy the live render state → g_snap (on core1, at VBlank, for a clean frame).
 * Uses the real globals (before the macro redirects); the affine is saved
 * separately because the render re-accumulates from it. */
extern "C" void render_snap_capture(void)
{
  if (!g_snap) return;
  memcpy(g_snap,                          gbsp_vram,             SNAP_VRAM_BYTES);
  memcpy(g_snap + SNAP_VRAM_BYTES,        palette_ram_converted, 512 * 2);
  memcpy(g_snap + SNAP_VRAM_BYTES + 1024, oam_ram,               512 * 2);
  memcpy(g_snap + SNAP_VRAM_BYTES + 2048, io_registers,          512 * 2);
  g_snap_affx[0] = affine_reference_x[0]; g_snap_affx[1] = affine_reference_x[1];
  g_snap_affy[0] = affine_reference_y[0]; g_snap_affy[1] = affine_reference_y[1];
}

extern "C" int render_offload_init(void)
{
  if (!g_snap)
  {
#ifdef ESP_PLATFORM
    g_snap = (u8 *)heap_caps_malloc(SNAP_TOTAL_BYTES, MALLOC_CAP_SPIRAM);
#else
    g_snap = (u8 *)malloc(SNAP_TOTAL_BYTES);
#endif
  }
  return g_snap != NULL;
}

/* ----- From here the render functions read/write via rc, a local render_ctx*
 * that each function fetches with 'render_ctx *const rc = RENDER_RC();'. The call
 * sites are untouched (the names are now macros). read_ioreg32 follows read_ioreg
 * automatically (use-time expansion). ----- */
#undef vram
#define vram (*(R_CTX.vram))
#undef read_ioreg
#define read_ioreg(regnum) (eswap16(R_CTX.io[(regnum)]))
#undef write_ioreg
#define write_ioreg(regnum, val) (R_CTX.io[(regnum)] = eswap16(val))
#undef read_dmareg
#define read_dmareg(regnum, dmachan) (eswap16(R_CTX.io[(regnum) + (dmachan) * 6]))
#undef write_dmareg
#define write_dmareg(regnum, dmachan, val) (R_CTX.io[(regnum) + (dmachan) * 6] = eswap16(val))
#define palette_ram_converted (R_CTX.pal)
#define oam_ram (R_CTX.oam)
#define affine_reference_x (R_CTX.affx)
#define affine_reference_y (R_CTX.affy)
#undef get_screen_pixels
#define get_screen_pixels() (R_CTX.out)
#endif /* RENDER_OFFLOAD */
...
// Rebuilt every rendered frame (order_obj) and read back per scanline, so it
// tolerates PSRAM latency. Moved out of static BSS to free 100KB of internal
// SRAM for vram (allocated in main.c, same pointer-to-array trick as iwram/vram).
u8 (*gbsp_obj_priority_list)[5][160][128];
#define obj_priority_list (*gbsp_obj_priority_list)
...
#if RENDER_OFFLOAD
/* Render the snapshot (g_snap) → 'out' on the render core (≠ emu core), via the
 * snapshot ctx that bind_snap set. Whole frame from the end state; obj ordering from
 * the snapshot OAM (line 0). reg[OAM_UPDATED] is shared, but in the overworld
 * core1 doesn't render → harmless. */
extern "C" void render_offload_frame(u16 *out)
{
  render_ctx_bind_snap(out);
  reg[OAM_UPDATED] = 1;
  for (u32 v = 0; v < GBA_SCREEN_HEIGHT; v++)
  {
    write_ioreg(REG_VCOUNT, v);
    update_scanline();
  }
}
#endif
