...
static void input_task(void *arg)
{
    ...
    while (input_task_running)
    {
        if (rg_input_read_gamepad_raw(&state))
        {
            ...
            uint32_t current_volume_keys = local_gamepad_state & (RG_KEY_L | RG_KEY_R | RG_KEY_X | RG_KEY_Y);
            ...
            local_gamepad_state &= ~(RG_KEY_L | RG_KEY_R | RG_KEY_X | RG_KEY_Y); // Remove from gamepad state
