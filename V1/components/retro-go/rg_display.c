...
// Drain pending SPI work before touching margins. Without this, rapid clicks on
// the Screen position slider could exhaust the SPI transaction queue (10 slots)
// and trigger RG_PANIC("display") after a 2.5s timeout.
...
void rg_display_write_rect(int left, int top, int width, int height, int stride, const uint16_t *buffer, uint32_t flags)
{
    ...
    // <=0 (not just <0) catches the edge case left == display.screen.width.
    // Without the equals case, width=0 falls through and triggers an
    // IntegerDivideByZero on the `LCD_BUFFER_LENGTH / width` below.
    if (width <= 0 || height <= 0)
        return;
...
void rg_display_init(void)
{
    ...
    // Override compile-time visible area with NVS values from the Screen position menu.
