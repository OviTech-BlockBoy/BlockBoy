#pragma once

#include <stdbool.h>
#include <stdint.h>

typedef enum {
    NETPLAY_MODE_NONE,
    NETPLAY_MODE_HOST,
    NETPLAY_MODE_GUEST,
} netplay_mode_t;

typedef enum {
    NETPLAY_EVENT_PACKET_RECEIVED,
    NETPLAY_EVENT_STATUS_CHANGED,
    NETPLAY_EVENT_GAME_RESET,
} netplay_event_t;

typedef enum {
    NETPLAY_STATUS_NOT_INIT,
    NETPLAY_STATUS_STOPPED,
    NETPLAY_STATUS_DISCONNECTED,
    NETPLAY_STATUS_LISTENING,
    NETPLAY_STATUS_CONNECTING,
    NETPLAY_STATUS_HANDSHAKE,
    NETPLAY_STATUS_CONNECTED,
} netplay_status_t;

typedef enum {
    NETPLAY_PACKET_INFO,
    NETPLAY_PACKET_PING,
    NETPLAY_PACKET_PONG,
    NETPLAY_PACKET_READY,
    NETPLAY_PACKET_SYNC_REQ,
    NETPLAY_PACKET_SYNC_ACK,
    NETPLAY_PACKET_SYNC_DONE,
    NETPLAY_PACKET_RESYNC,
    NETPLAY_PACKET_GAME_START,
    NETPLAY_PACKET_GAME_PAUSE,
    NETPLAY_PACKET_GAME_RESET,
    NETPLAY_PACKET_INPUT,
    NETPLAY_PACKET_SERIAL,
    NETPLAY_PACKET_RAW_DATA,
} netplay_packet_type_t;

typedef struct __attribute__ ((packed)) {
    uint8_t player_id;
    uint8_t cmd;
    uint8_t arg;
    uint8_t data_len;
    uint8_t data[128];
} netplay_packet_t;

typedef struct __attribute__ ((packed)) {
    uint8_t  version;
    uint8_t  id;
    uint32_t game_id;
    uint16_t status;
    uint32_t ip_addr;
    uint32_t last_contact;
    uint8_t  sync_data[16];
} netplay_player_t;

typedef void (*netplay_callback_t)(netplay_event_t event, void *arg);
typedef netplay_callback_t rg_netplay_handler_t;

void rg_netplay_init(netplay_callback_t callback);
bool rg_netplay_quick_start(void);
bool rg_netplay_start(netplay_mode_t mode);
bool rg_netplay_stop(void);
void rg_netplay_sync(void *data_in, void *data_out, uint8_t data_len);

// ─── Game Link serial transport (frame protocol v2) ──────────────────────────────
// Every Game Boy serial byte travels as a four-byte frame over TCP:
//   [0] magic 0xA5   [1] type   [2] seq   [3] SB value
// The sequence number makes a late reply to an aborted transfer recognizable,
// so no bytes ever need to be thrown away.
// These values must equal GB_LINK_* in gnuboy.h.
#define RG_LINK_MASTER 1   // master clocks a byte out
#define RG_LINK_REPLY  2   // reply from the partner, carrying the master's seq

int  rg_netplay_link_send(uint8_t type, uint8_t seq, uint8_t data);
bool rg_netplay_link_recv(uint8_t *type, uint8_t *seq, uint8_t *data);
void rg_netplay_close_serial(void);

netplay_mode_t   rg_netplay_mode(void);
netplay_status_t rg_netplay_status(void);
