...
static rg_gui_event_t font_type_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    else if (event == RG_DIALOG_NEXT && rg_gui_set_font(gui.font_index + 1))
        result = RG_DIALOG_REDRAW;
...
// to the main menu instead of showing a URL that can't answer.
...
// measured and drawn; the strings live in static buffers so their pointers stay
...
void rg_gui_options_menu(void)
{
    ...
    if (app_check->isLauncher)
    {
        ...
        // Launcher colors + launcher behaviour are inserted by the handler on tab 0
        ...
        #endif
        // File server + USB Mode are inserted by the handler on tab 1

        // -------- Launcher handler vult tab 0 (launcher items) en tab 1 (rest van connectivity) --------
    ...
    // Service-only Screen position: hidden by default, toggle via Shop menu.
...
// Pointer to currently-displayed tabbed dialog's entries (lets service_cb reach the registry).
static rg_gui_menu_entry_t *current_tabbed_entries = NULL;
// Set by callbacks that change visible entries (Shop Menu, sub-dialogs).
...
// Fill visible[] with indices of entries belonging to tab_id that pass visibility check.
                                  ...
                                  int tab_id, int *visible)
{
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        if ((entries[i].option.flags & RG_DIALOG_FLAG_MODE_MASK) == RG_DIALOG_FLAG_HIDDEN)
            continue;
        // Dynamic visibility -- Shop Menu toggles take effect immediately
...
// Compute the container's top y, based on whether a title sits above it.
// Used by tabbed_draw_chrome (drawing the container) and tabbed_draw (drawing tabs/items).
static int tabbed_box_y(const char *title)
{
    if (!title) return 12;  // margin
    const int title_h = gui.style.font_height + 6;
    return 4 + title_h + 2;  // top_gap + title_h + title_to_box
}

// Draws the dialog chrome: screen wipe + title (above container if present) + outer box border.
// Called once at startup and again whenever tabbed_force_full_redraw is set (after nested popups).
static void tabbed_draw_chrome(const char *title)
{
    ...
    // Title above container (in the strip above box_y)
    if (title)
    {
                        ...
                        bool full_redraw, int prev_sel)
{
    ...
    // Title is drawn ABOVE the container in tabbed_draw_chrome; container sits lower when a title is present.
    const int box_y = tabbed_box_y(title);
    ...
    const int tabs_y = box_y + 2;  // just inside the 2px top container border
    ...
    if (!partial_ok)
    {
        // Tab bar — 2px margin l/r so the container border stays intact.
        // All tabs solid filled; 1px box_border separators between tabs (no overlap with the container border).
        ...
        // 1px divider between every two adjacent tabs
    ...
    {
        // Partial redraw: repaint only the two rows whose state changed.
                              ...
                              int initial_tab)
{
    ...
    // Allocate value buffers so callbacks can overwrite option->value safely
    ...
    // One-time chrome: screen wipe + title (above container) + outer box border
    ...
    while (event != RG_DIALOG_SELECT && event != RG_DIALOG_CANCEL)
    {
        if (redraw)
        {
            // Visibility may have changed -- clamp sel + repaint chrome if needed
        ...
        if (joystick ^ joystick_old)
        {
            ...
            else if (vcount > 0 && sel < vcount)
            {
                ...
                if (joystick & RG_KEY_LEFT && cb) {
                    ...
                    // ENTER callbacks may open nested rg_gui_dialog popups (Language, Theme,
                    // Timezone, Wi-Fi, ...). Those overwrite pixels inside the tabbed menu,
                    // so force a full redraw of the chrome+items area on return — otherwise
                    // the row under the popup keeps the popup's pixels visible.
