...
static rg_gui_event_t color_theme_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    // Always update the displayed value, also on PREV/NEXT — otherwise the
    // menu shows the old "N/4" text after cycling and it looks broken.
...
static rg_gui_event_t showcase_select_games_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    // pressing A on them returns their arg (RG_DIALOG_SELECT); games toggle in place.
    {
        ...
        while (1)
        {
            ...
            char title[48];
            snprintf(title, sizeof(title), "%s (%d/%d)", _("Select games"), page + 1, pages);
...
// dialog starts the file server for this session (not persisted -- the toggle
// below is what makes it survive a reboot) and shows the password it will ask for.
...
static void retro_loop(void)
{
    ...
    // schedule enabled: a timer wake (daily On time) (re)starts showcase; any
    // other wake (button / power-on) stops showcase AND disarms the schedule
    // (SchedEnable=0) so it can't sleep the device mid-game later.
#ifdef ESP_PLATFORM
    if (app->isColdBoot && rg_settings_get_number(NS_GLOBAL, "SchedEnable", 0))
    {
        ...
        else
        {
            ...
            // it can't sleep the device while the user is playing.
            rg_settings_set_number(NS_GLOBAL, "DisplayActive", 0);
            rg_settings_set_number(NS_GLOBAL, "DisplayElapsedSec", 0);
            rg_settings_set_number(NS_GLOBAL, "SchedEnable", 0);
        }
...
// Fills the tabbed settings dialog from the launcher domain.
// tab_id: 0=General, 1=System.
// To move an item to a different tab: change its tab_id.
// To change default visibility for customers: flip default_vis.
...
#ifdef ESP_PLATFORM
// Guard against flashing the wrong BlockBoy variant. V1 (N4R2) ships with a
// 4MB flash chip; V2 (N16R8) has 16MB. FLASHSIZE_DETECT is enabled, so
// esp_flash_get_size() returns the real chip size regardless of build config.
// A mismatch means the user flashed the firmware for the other model -- show a
// clear message instead of crashing cryptically or running misconfigured.
#define BLOCKBOY_EXPECTED_FLASH_MB 4
static void check_hardware_variant(void)
{
    uint32_t flash_size = 0;
    if (esp_flash_get_size(NULL, &flash_size) != ESP_OK)
        return; // can't read the chip -- don't block boot
    if ((flash_size / (1024 * 1024)) == BLOCKBOY_EXPECTED_FLASH_MB)
        return; // correct firmware for this hardware

    int w = rg_display_get_width();
    int h = rg_display_get_height();
    rg_display_clear(C_RED);
    rg_gui_draw_text(0, h / 2 - 48, w, "WRONG FIRMWARE",
                     C_WHITE, C_RED, RG_TEXT_ALIGN_CENTER | RG_TEXT_BIGGER);
    rg_gui_draw_text(0, h / 2 - 8, w,
                     "This is a BlockBoy V2 (N16R8).\n"
                     "You flashed the V1 firmware.\n"
                     "Please flash the V2 version.",
                     C_WHITE, C_RED, RG_TEXT_ALIGN_CENTER | RG_TEXT_MULTILINE);
    while (1)
        rg_task_delay(1000); // halt -- do not continue booting
}
#endif
