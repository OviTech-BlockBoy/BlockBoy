...
// When false (default) the directory scan runs silently. This keeps showcase
// game-to-game transitions and boot smooth (no "Scanning ... N" counter flashing).
// Only the interactive "Select games" screen turns it on for feedback.
static bool scan_progress_visible = false;
void application_set_scan_progress(bool enable) { scan_progress_visible = enable; }
...
static int scan_folder_cb(const rg_scandir_t *entry, void *arg)
{
    ...
    // Show a running count so a long scan (many ROMs on a slow SD card) is
    // visibly progressing instead of looking frozen. Throttled to keep the
    // draw cost off the scan's critical path.
    if (scan_progress_visible && (app->files_count & 63) == 0)
        rg_gui_draw_message(_("Scanning %s... %d"), app->short_name, app->files_count);
...
void application_init(retro_app_t *app)
{
...
// Public path-based launcher used by the web UI's "Play" command. Resolves the
// ROM path to its emulator and boots it. On success this reboots and does not
// return; returns false only if the path doesn't belong to any known emulator.
bool application_start_path(const char *path)
{
    retro_file_t file;
    if (!application_path_to_file(path, &file))
        return false;
...
retro_app_t *application_find(const char *short_name)
{
    if (!short_name)
        return NULL;
    for (int i = 0; i < apps_count; i++)
    {
        if (strcmp(apps[i]->short_name, short_name) == 0)
            return apps[i];
    }
    return NULL;
}

void application_start_by_index(retro_app_t *app, int index)
{
    RG_ASSERT_ARG(app);
    ...
    if (index < 0 || index >= (int)app->files_count)
        return;

    retro_file_t *file = &app->files[index];
    if (file->type != RETRO_TYPE_FILE)
        return;

    char *part = strdup(app->partition);
    char *name = strdup(app->short_name);
    char *path = strdup(get_file_path(file));
    int flags = RG_BOOT_ONCE;
