...
*/
void gnuboy_set_serial_callback(gb_serial_send_cb_t *send_cb, gb_serial_recv_cb_t *recv_cb)
{
	GB.link.send_cb         = send_cb;
	GB.link.recv_cb         = recv_cb;
	GB.link.disconnect_cb   = NULL;
	GB.link.no_partner      = (send_cb == NULL);
	GB.link.is_master       = false;
	GB.link.transfer_active = false;
	GB.link.wait_count      = 0;
	// Fresh session: don't carry over a sequence number, held byte or poll
	// state from a previous connection (see bug #1 in the Game Link guide).
	GB.link.seq             = 0;
	GB.link.hold_valid      = false;
	GB.link.hold_time       = 0;
	GB.link.poll_timer      = 0;
}

void gnuboy_set_serial_disconnect_cb(void (*cb)(void))
{
	GB.link.disconnect_cb = cb;
}

void gnuboy_set_no_partner(bool no_partner)
{
	GB.link.no_partner = no_partner;
}
...
void gnuboy_load_bank(int bank)
{
	...
	for (int attempt = 0; !cart.rombanks[bank]; attempt++)
	{
		if (attempt >= cart.romsize * 2)
		{
			MESSAGE_ERROR("No reclaimable bank found for bank %d\n", bank);
			break; // fread(NULL) would crash — handled by ok-check below
		}
		int i = rand() % cart.romsize; // was: & 0xFF — out-of-bounds for small ROMs
		if (cart.rombanks[i])
		{
	...
	// Load the 16K page, retry up to 3x for transient SD card errors
	bool ok = false;
	for (int retry = 0; retry < 3 && !ok; retry++)
	{
		if (retry > 0)
			MESSAGE_WARN("ROM bank %d retry %d\n", bank, retry);
		ok = (fseek(cart.romFile, OFFSET, SEEK_SET) == 0
		      && fread(cart.rombanks[bank], BANK_SIZE, 1, cart.romFile) == 1);
	}
	if (!ok)
	{
		MESSAGE_WARN("ROM bank %d load failed after retries\n", bank);
		if (!feof(cart.romFile))
			abort(); // Persistent SD card failure
	}
}
