	...
	case 0xE000: // Memory mapped IO
		...
		else
		{
			...
			case RI_SC:
				...
				if (b & 0x80)
				{
					// On a CGB, SC bit 1 selects the fast serial clock (262144 Hz
					// instead of 8192 Hz). That used to be ignored: a GBC title
					// using the fast clock was served 32x too slowly, and without
					// a partner its 0xFF arrived 32x too late.
					int period = (IS_CGB && (b & 0x02)) ? GB_SERIAL_PERIOD_FAST
					                                   : GB_SERIAL_PERIOD;
					if (hw.link.send_cb)
					{
						hw.link.is_master       = (b & 0x01);
						hw.link.transfer_active = true;
						hw.link.wait_count      = 0;
						// Pump immediately instead of waiting for the next idle
						// poll: if a byte is already waiting, the game gets it now.
						hw.link.poll_timer      = 0;
						if (hw.link.is_master)
						{
							// Every master transfer gets a new sequence number. A reply
							// to an earlier, aborted transfer carries an old number and
							// is ignored by link_pump(). So the receive buffer no longer
							// needs to be flushed here: that flush also ate the fresh
							// bytes of a game sending several in a row (handshake,
							// checksum, level seed).
							hw.link.send_cb(GB_LINK_MASTER, ++hw.link.seq, R_SB);
						}
						hw.serial = period;
					}
					else
					{
						// No partner: behave like hardware without a cable.
						hw.link.transfer_active = false;
						if ((b & 0x01))
							hw.serial = period;
						else
							hw.serial = hw.link.no_partner ? 1 : 0;
					}
				}
				else
				{
