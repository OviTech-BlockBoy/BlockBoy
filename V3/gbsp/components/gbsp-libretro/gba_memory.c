...
#include "rg_storage.h"   /* rg_storage_rawrom_* (direct multi-block ROM page reads) */
#endif
...
/* Pages (32KB) per cache block. Full blocks = 32; the last block may be
 * partial (<32) to use free PSRAM as extra cache. */
u16 gamepak_block_pages[32];
u32 gamepak_total_pages;    /* som van gamepak_block_pages[0..count-1] */
...
#if RENDER_OFFLOAD
/* Mid-frame dirty detector: marks per category when a raster-relevant write
 * happens during active display (vcount<160). bits: 1=IO 2=palette 4=VRAM 8=OAM 16=DMA. */
u32 g_m2_dirty;
/* 1 = previous frame was clean (no mid-frame changes). */
u32 g_predict_clean;
/* 1 = render-to-core1 offload active (only at 1.0x without frameskip). */
u32 g_offload_active;
#define M2_MARK_WRITE(addr) do {                                              \
    u32 _r = (addr) >> 24;                                                    \
    if (_r >= 0x04 && _r <= 0x07 && read_ioreg(REG_VCOUNT) < 160) {           \
      if      (_r == 0x06) g_m2_dirty |= 4;          /* VRAM        */        \
      else if (_r == 0x07) g_m2_dirty |= 8;          /* OAM         */        \
      else if (_r == 0x05) g_m2_dirty |= 2;          /* palette     */        \
      else if (((addr) & 0x3FF) < 0x58) g_m2_dirty |= 1; /* raster IO */      \
    }                                                                         \
  } while (0)
#else
#define M2_MARK_WRITE(addr) do {} while (0)
#endif
...
/* Bulk write for the LZ77 HLE (cpu.cpp). Writes len bytes from src to GBA address dst
 * in one memcpy when the destination is a flat region (EWRAM/IWRAM; VRAM only when
 * vram_ok — the 16-bit SWI variant, since byte writes to VRAM have the duplication
 * quirk). Side effects identical to write_memory*: EWRAM/IWRAM have none
 * (SMC_DETECTION=0), VRAM gets the dirty mark. Returns 0 = not handled → caller falls
 * back to the per-unit handler loop (byte-exact safety net for mirrors/region edges). */
int gba_bulk_write(u32 dst, const u8 *src, u32 len, int vram_ok)
{
  u8 *base; u32 off, lim;
  switch (dst >> 24)
  {
    case 0x02: base = ewram; off = dst & 0x3FFFF; lim = 0x40000; break;
    case 0x03: base = iwram + (0x8000 * SMC_DETECTION); off = dst & 0x7FFF; lim = 0x8000; break;
    case 0x06:
      if (!vram_ok) return 0;
      off = dst & 0x1FFFF;
      if (off >= 0x18000) off -= 0x8000;
      base = vram; lim = 0x18000; break;
    default: return 0;
  }
  if (off + len > lim)
    return 0;
...
/* ROM prefetch: low-prio background task that reads the next 32KB page into a
 * staging buffer while the game runs; on a miss the main path copies from
 * staging instead of the slow SD read. Dormant (0); only marginal gain. */
#define ROM_PREFETCH 0
#if ROM_PREFETCH && defined(ESP_PLATFORM)
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "freertos/semphr.h"
#include "esp_heap_caps.h"

static char            rp_path[512];
static u8             *rp_stage;          /* 32KB PSRAM staging */
static volatile u32    rp_stage_page;
static volatile int    rp_stage_ready;
static FILE           *rp_file;           /* separate ROM handle for the task */
static SemaphoreHandle_t rp_mtx;          /* serialiseert SD-toegang + staging */
static TaskHandle_t    rp_task;

static void rom_prefetch_task(void *arg)
{
  for (;;)
  {
    uint32_t page;
    xTaskNotifyWait(0, 0xFFFFFFFFu, &page, portMAX_DELAY);
    if (page >= (gamepak_size >> 15)) continue;
    if (!rp_stage || (rp_stage_ready && rp_stage_page == page)) continue;

    /* Read in 4KB chunks and release the mutex each chunk so the high-prio
     * main path doesn't wait long. ready only after the whole page. */
    int ok = 1;
    for (u32 off = 0; off < 32 * 1024; off += 4096)
    {
      xSemaphoreTake(rp_mtx, portMAX_DELAY);
      if (!rp_file) rp_file = fopen(rp_path, "rb");
      if (rp_file)
      {
        fseek(rp_file, page * (32 * 1024) + off, SEEK_SET);
        if (fread(rp_stage + off, 4096, 1, rp_file) != 1) ok = 0;
      }
      else ok = 0;
      xSemaphoreGive(rp_mtx);          /* hoofdpad mag ertussen */
      if (!ok) break;
    }
    if (ok) { rp_stage_page = page; rp_stage_ready = 1; }
  }
}

static void rom_prefetch_init(void)
{
  if (rp_task || rp_path[0] == 0) return;          /* eenmalig, ná ROM-open */
  rp_stage = (u8 *)heap_caps_malloc(32 * 1024, MALLOC_CAP_SPIRAM);
  rp_mtx   = xSemaphoreCreateMutex();
  if (rp_stage && rp_mtx)
    xTaskCreatePinnedToCore(rom_prefetch_task, "rom_prefetch", 4096, NULL,
                            1 /* lage prio */, &rp_task, 0 /* core0 */);
}
#endif

/* Fast ROM read via direct sdmmc_read_sectors (FatFS bypassed), in
 * rg_storage_rawrom_* (see rg_storage.c). g_rawrom handle + fread fallback. */
#if defined(ESP_PLATFORM)
#define ROM_RAW_READ 1                 /* master switch; 0 = fread behaviour */
static rg_rawrom_t *g_rawrom = NULL;
#else
#define ROM_RAW_READ 0
#endif

/* Use free PSRAM above the offload reserve as one partial (sub-1MB) cache block.
 * 0 = no partial block (uniform 32 pages/block). */
#define ROM_PARTIAL_BLOCK 1

/* What must stay free AFTER the partial block, for the RUNNING workload: the offload
 * snapshot (~99KB, allocated after the ROM cache) + transient margin (menu/screenshot/
 * file I/O). The 416KB savestate peak is deliberately NOT reserved here anymore: the
 * partial block is pure cache and yields temporarily during save/load state
 * (gamepak_cache_release/restore_partial, wired in the main.c state handlers). */
#define ROM_PARTIAL_KEEP_FREE (256 * 1024)
static int g_partial_present = 0;   /* 1 = partial block allocated (last buffer slot) */

/* Map an LRU page index (0..gamepak_total_pages-1) to the address of that 32KB page.
 * Full blocks = 32 pages; an optional partial last block < 32 → variable mapping. */
static inline u8 *gamepak_page_ptr(u32 entry)
{
  u32 b = 0;
  while (entry >= gamepak_block_pages[b]) { entry -= gamepak_block_pages[b]; b++; }
  return &gamepak_buffers[b][(u32)(32 * 1024) * entry];
}
...
u8 *load_gamepak_page(u32 physical_index)
{
  ...
  u8 *swap_location = gamepak_page_ptr(entry);
...
#if ROM_PREFETCH && defined(ESP_PLATFORM)
  rom_prefetch_init();
  if (rp_mtx) xSemaphoreTake(rp_mtx, portMAX_DELAY);
  if (rp_stage && rp_stage_ready && rp_stage_page == physical_index)
  {
    memcpy(swap_location, rp_stage, 32 * 1024);   /* prefetch hit: no slow SD read */
    rp_stage_ready = 0;
    rp_from_stage = 1;
  }
#endif
  if (!rp_from_stage)
  {
    int got = 0;
#if ROM_RAW_READ
    /* Direct multi-block sdmmc read into an internal DMA buffer + memcpy to PSRAM
     * (FatFS/fread bypassed). If this fails → fread fallback below. */
    if (g_rawrom)
      got = rg_storage_rawrom_read(g_rawrom, physical_index * (32 * 1024),
                                   swap_location, (32 * 1024));
#endif
    if (!got)
    {
      fseek(gamepak_file_large, physical_index * (32 * 1024), SEEK_SET);
      fread(swap_location, (32 * 1024), 1, gamepak_file_large);   /* fallback: single-block ~54ms */
    }
  }
#if ROM_PREFETCH && defined(ESP_PLATFORM)
  if (rp_mtx) xSemaphoreGive(rp_mtx);
  /* request the next page ahead — walking around = sequential ROM access */
  if (rp_task && physical_index + 1 < (gamepak_size >> 15))
    xTaskNotify(rp_task, physical_index + 1, eSetValueWithOverwrite);
#endif
...
void init_gamepak_buffer(void)
{
  ...
  {
#if (RENDER_OFFLOAD && defined(ESP_PLATFORM))
    /* Leave PSRAM margin free for the offload snapshot + savestate buffer: stop
     * ROM caching if the next 1MB chunk would eat into that margin. */
    if (heap_caps_get_free_size(MALLOC_CAP_SPIRAM) <
        gamepak_buffer_blocksize + RENDER_OFFLOAD_PSRAM_RESERVE)
      break;
#endif
    void *ptr = malloc(gamepak_buffer_blocksize);
    if (!ptr)
      break;
    gamepak_block_pages[gamepak_buffer_count] = 32;          /* full block */
    gamepak_buffers[gamepak_buffer_count++] = (u8*)ptr;
  }

#if (ROM_PARTIAL_BLOCK && RENDER_OFFLOAD && defined(ESP_PLATFORM))
  /* Use PSRAM above ROM_PARTIAL_KEEP_FREE as one partial (sub-1MB) cache block.
   * The savestate peak is no longer reserved permanently (see ROM_PARTIAL_KEEP_FREE),
   * which typically yields ~10 extra cache pages. */
  if (gamepak_buffer_count < ROM_BUFFER_SIZE)
  {
    size_t freeb = heap_caps_get_free_size(MALLOC_CAP_SPIRAM);
    if (freeb > ROM_PARTIAL_KEEP_FREE)
    {
      u32 pages = (u32)((freeb - ROM_PARTIAL_KEEP_FREE) / (32 * 1024));
      if (pages > 31) pages = 31;                            /* >1MB would already be taken by the greedy loop */
      if (pages >= 1)
      {
        void *ptr = heap_caps_malloc((size_t)pages * 32 * 1024, MALLOC_CAP_SPIRAM);
        if (ptr)
        {
          gamepak_block_pages[gamepak_buffer_count] = (u16)pages;
          gamepak_buffers[gamepak_buffer_count++] = (u8*)ptr;
          g_partial_present = 1;
        }
      }
  ...
  // total pages over alle (volle + evt. partiële) blokken
  gamepak_total_pages = 0;
  for (i = 0; i < gamepak_buffer_count; i++)
    gamepak_total_pages += gamepak_block_pages[i];
  ...
  gamepak_lru_head = 0;
  gamepak_lru_tail = (u16)(gamepak_total_pages - 1);
}
...
bool gamepak_must_swap(void)
{
  ...
  return gamepak_total_pages * (u32)(32 * 1024) < gamepak_size;
}

/* Savestate-reserve trick: temporarily release the partial cache block so the big
 * transient savestate buffer fits, then put it back. ROM cache is pure cache:
 * released pages simply re-fault from SD. Call order (main.c state handlers, with
 * emulation paused): release -> alloc/save/free -> restore. The LRU chain is simply
 * re-threaded (order history is expendable; full-block phy_rom mappings stay intact). */
u32 gamepak_cache_release_partial(void)
{
#if (ROM_PARTIAL_BLOCK && RENDER_OFFLOAD && defined(ESP_PLATFORM))
  if (!g_partial_present)
    return 0;
  u32 pbi = gamepak_buffer_count - 1;
  u32 bp  = gamepak_block_pages[pbi];
  u32 base = gamepak_total_pages - bp;
  for (u32 e = base; e < gamepak_total_pages; e++)
  {
    s16 phy = gamepak_blk_queue[e].phy_rom;
    if (phy >= 0)
      map_rom_entry(read, phy, NULL, gamepak_size >> 15);   /* mirror-safe unmap */
    gamepak_blk_queue[e].phy_rom = -1;
  }
  free(gamepak_buffers[pbi]);
  gamepak_buffers[pbi] = NULL;
  gamepak_block_pages[pbi] = 0;
  gamepak_buffer_count--;
  gamepak_total_pages = base;
  for (u32 i = 0; i < 1024; i++)
    gamepak_blk_queue[i].next_lru = (u16)(i + 1);
  gamepak_lru_head = 0;
  gamepak_lru_tail = (u16)(base - 1);
  g_partial_present = 0;
  return bp * (u32)(32 * 1024);
#else
  return 0;
#endif
}

void gamepak_cache_restore_partial(void)
{
#if (ROM_PARTIAL_BLOCK && RENDER_OFFLOAD && defined(ESP_PLATFORM))
  if (g_partial_present || gamepak_buffer_count == 0 ||
      gamepak_buffer_count >= ROM_BUFFER_SIZE)
    return;
  size_t freeb = heap_caps_get_free_size(MALLOC_CAP_SPIRAM);
  if (freeb <= ROM_PARTIAL_KEEP_FREE + (32 * 1024))
    return;
  u32 pages = (u32)((freeb - ROM_PARTIAL_KEEP_FREE) / (32 * 1024));
  if (pages > 31) pages = 31;
  void *ptr = heap_caps_malloc((size_t)pages * 32 * 1024, MALLOC_CAP_SPIRAM);
  if (!ptr)
    return;                          /* no disaster: just less cache until game switch */
  u32 base = gamepak_total_pages;
  gamepak_block_pages[gamepak_buffer_count] = (u16)pages;
  gamepak_buffers[gamepak_buffer_count++] = (u8*)ptr;
  gamepak_total_pages = base + pages;
  for (u32 e = base; e < gamepak_total_pages; e++)
    gamepak_blk_queue[e].phy_rom = -1;          /* fresh empty cache pages */
  for (u32 i = 0; i < 1024; i++)
    gamepak_blk_queue[i].next_lru = (u16)(i + 1);
  gamepak_lru_head = 0;
  gamepak_lru_tail = (u16)(gamepak_total_pages - 1);
...
{
#if ROM_RAW_READ
  if (g_rawrom) { rg_storage_rawrom_close(g_rawrom); g_rawrom = NULL; }
#endif
...
#if ROM_RAW_READ
  if (g_rawrom) { rg_storage_rawrom_close(g_rawrom); g_rawrom = NULL; }  /* game-switch */
  g_rawrom = rg_storage_rawrom_open(name);   /* NULL = bypass off, fread fallback still works */
#endif
#if ROM_PREFETCH && defined(ESP_PLATFORM)
  strncpy(rp_path, name, sizeof(rp_path) - 1);   /* for the prefetch task's own handle */
  rp_path[sizeof(rp_path) - 1] = 0;
  if (rp_mtx)                                    /* game-switch: reset prefetch-state */
  {
    xSemaphoreTake(rp_mtx, portMAX_DELAY);
    if (rp_file) { fclose(rp_file); rp_file = NULL; }
    rp_stage_ready = 0;
    xSemaphoreGive(rp_mtx);
  }
#endif
  if(gamepak_file_large)
  {
    ...
    // Proceed to read the whole ROM or as much as possible. Per-block page count is
    // variable (last block may be partial) → read bp*32KB and map base_page..base_page+bp.
    u32 base_page = 0;
    for (i = 0; i < ldblks; i++)
    {
      u32 bp = gamepak_block_pages[i];
      // Load the block (bp * 32KB; = 1MB for full blocks) and map it
      fread(gamepak_buffers[i], 32 * 1024, bp, gamepak_file_large);
      for (j = 0; j < bp && base_page + j < rom_blocks; j++)
      {
        u32 phyn = base_page + j;
