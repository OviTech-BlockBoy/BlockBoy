...
// Everything the firmware owns on the SD card lives here. Cards made before
// the rename carry "/retro-go"; rg_storage_init() renames such a folder once
// on boot (see migrate_base_path) so saves and settings survive an update.
#define RG_BASE_PATH        RG_STORAGE_ROOT "/BlockBoy"
#define RG_BASE_PATH_LEGACY RG_STORAGE_ROOT "/retro-go"
...
// §6.60 ROM-swap-hik fix: a "raw ROM" handle that reads file pages via DIRECT multi-block
// sdmmc_read_sectors (CMD18) into an internal DMA buffer, bypassing FatFS/fread. This avoids
// the v5.5 single-block fallback the SDSPI driver uses for PSRAM destinations (TEST 1: 2.14x
// faster). The handle keeps a FatFS file open + a fast-seek cluster link map (CLMT) so any
// file offset maps to an absolute card LBA without per-page FAT walks. dst may be in PSRAM.
//
// CONCURRENCY: rawrom_read bypasses the VFS lock. It is safe ONLY while the SD card has a
// single accessor (during gameplay: emulation on core0; render-offload on core1 never touches
// SD; savestates run with emulation paused). Do not call concurrently with other SD access.
typedef struct rg_rawrom rg_rawrom_t;
rg_rawrom_t *rg_storage_rawrom_open(const char *vfs_path);                       // NULL on failure
bool         rg_storage_rawrom_read(rg_rawrom_t *handle, uint32_t file_offset,   // offset+len must be
                                    void *dst, uint32_t len);                    //  512-byte multiples
void         rg_storage_rawrom_close(rg_rawrom_t *handle);
