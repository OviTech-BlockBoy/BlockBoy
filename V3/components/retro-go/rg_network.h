...
{
    char ssid[32 + 1];
    int rssi;       // signal strength in dBm (negative; closer to 0 is stronger)
    int channel;
    int auth_mode;  // 0 = open, non-zero = secured (esp-idf wifi_auth_mode_t)
} rg_wifi_scan_result_t;
...
// Scan nearby networks (caller allocates 'results'). Returns count (0..max) or -1.
