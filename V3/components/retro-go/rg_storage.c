...
rg_rawrom_t *rg_storage_rawrom_open(const char *vfs_path)
{
    ...
    FRESULT fr = f_open(&h->fil, ffpath, FA_READ);
    if (fr != FR_OK)
    {
        // Log the code: 1=DISK_ERR, 4=NO_FILE, 5=NO_PATH, 17=NOT_ENOUGH_CORE,
        // 18=TOO_MANY_OPEN_FILES. Tells memory pressure apart from a bad path.
        RG_LOGW("[RAWROM] f_open('%s') failed, FRESULT=%d (free int=%d, largest=%d)", ffpath, (int)fr,
