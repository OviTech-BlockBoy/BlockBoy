...
// A font stores its glyphs as a variable-length stream, so finding one meant
// walking that stream from the start for every single character -- ~190 entries
// deep for the accented tail of the table. That cost is invisible for a label or
// two, but rg_gui_draw_dialog measures *every* row on *every* redraw, so a dialog
// with one row per ROM (Pick ROM, showcase "Select games") spent tens of
// milliseconds per keypress just re-walking the font. Every bundled font stays
// within U+0000..U+00FF, so a 256-entry pointer table covers them exactly;
// anything above that still falls back to the linear scan.
static const rg_font_t *glyph_index_font;
static const rg_font_glyph_t *glyph_index[256];

static void build_glyph_index(const rg_font_t *font)
{
...
static size_t get_glyph(uint32_t *output, const rg_font_t *font, int points, int c)
{
    ...
    const rg_font_glyph_t *glyph;

    if (c > 0 && c < 256)
    {
        if (font != glyph_index_font)
            build_glyph_index(font);
        glyph = glyph_index[c];
    }
    else
    {
                           ...
                           rg_color_t color_fg, rg_color_t color_bg, uint32_t flags)
{
    ...
    for (const char *ptr = text; *ptr;)
    {
        ...
        while (x_offset < draw_width)
        {
            ...
            // A dummy draw only needs the advance width, so don't pay for rendering
            // the glyph into `bitmap` (a memset plus a width*height bit loop per
            // character). TEXT_RECT() runs this loop for every row of every dialog
            // redraw, which is where long lists used to lose most of their time.
            int glyph_width = get_glyph((flags & RG_TEXT_DUMMY_DRAW) ? NULL : bitmap, font,
                                        font_height, rg_utf8_get_codepoint(&ptr));
...
// Measuring every row costs O(rows), and moving the cursor redraws the whole box —
// so a 600-row list (Pick ROM, showcase "Select games") re-measured 600 labels for
// every single step of the cursor. Nothing about the layout changes when only `sel`
// moves, so rg_gui_dialog() hands that case down here and we reuse the previous
// measurements. Only rg_gui_dialog may ask for reuse, and only after it has drawn
// the same array at least once itself: it is the only caller that knows whether an
// update_cb has rewritten a label or value in place since. Every other caller
// (rg_netplay's status box, magnet_launch's live readings) always re-measures.
static struct
{
    ...
    int font_height;
    int max_inner_width;
    int inner_width, col1_width, col2_width;
    int box_width, box_height;
    int *row_height;
} layout_cache;

static void draw_dialog(const char *title, const rg_gui_option_t *options, int sel, bool reuse_layout);

void rg_gui_draw_dialog(const char *title, const rg_gui_option_t *options, int sel)
{
    draw_dialog(title, options, sel, false);
}
...
static void draw_dialog(const char *title, const rg_gui_option_t *options, int sel, bool reuse_layout)
{
    ...
    // Everything the cache is keyed on must match, otherwise the stored measurements
    // describe a different box and we measure again.
    reuse_layout = reuse_layout && layout_cache.row_height && layout_cache.options == options &&
                   layout_cache.title == title && layout_cache.count == options_count &&
                   layout_cache.font == gui.style.font && layout_cache.font_height == font_height &&
                   layout_cache.max_inner_width == max_inner_width;

    int box_width, box_height, inner_width, col1_width, col2_width;
    int *row_height;

    if (reuse_layout)
    {
        box_width = layout_cache.box_width;
        box_height = layout_cache.box_height;
        inner_width = layout_cache.inner_width;
        col1_width = layout_cache.col1_width;
        col2_width = layout_cache.col2_width;
        row_height = layout_cache.row_height;
    }
    else
    {
        box_width = box_padding * 2;
        box_height = box_padding * 2 + (title ? font_height + 6 : 0);
        inner_width = TEXT_RECT(title, 0).width;
        col1_width = -1;
        col2_width = -1;
        ...
        row_height = calloc(options_count ?: 1, sizeof(int));
        if (!row_height)
            return;
        ...
        // drawn at inner_width (values at col2_width), which is narrower as soon
        // as no row needs the full width. Text then wraps onto more lines than we
        // budgeted for, the box comes out too short, and the bottom row is drawn
        // half over the border instead of being pushed to the next page. Measure
        // again with exactly the widths the draw loop below uses.
        box_height = box_padding * 2 + (title ? font_height + 6 : 0);
        for (size_t i = 0; i < options_count; i++)
        {
            rg_rect_t label = {0, 0, 0, min_row_height};
        ...
        // Hand the array to the cache instead of freeing it at the end.
        free(layout_cache.row_height);
        layout_cache = (__typeof__(layout_cache)){
            .options = options, .title = title, .count = options_count,
            .font = gui.style.font, .font_height = font_height,
            .max_inner_width = max_inner_width, .inner_width = inner_width,
            .col1_width = col1_width, .col2_width = col2_width,
            .box_width = box_width, .box_height = box_height, .row_height = row_height,
        };
    }
    ...
    // row_height is owned by layout_cache now; it is freed on the next re-measure.
}
...
intptr_t rg_gui_dialog(const char *title, const rg_gui_option_t *options_const, int selected_index)
{
    ...
    // Set whenever a callback may have rewritten a label or value in place, which is
    // the only thing that can change the box layout while the dialog is up. While it
    // is clear, draw_dialog() reuses the measurements it already took.
    ...
    // Held-key repeat. The first repeat waits RG_DIALOG_REPEAT_DELAY, then each
    // further one comes sooner, down to RG_DIALOG_REPEAT_MIN -- otherwise a 600-row
    // ROM list moves one row per 300ms and takes three minutes to walk through.
    // Acceleration applies to UP/DOWN only: holding A or LEFT/RIGHT re-fires an
    // option's callback, and that should stay at the original deliberate pace.
    #define RG_DIALOG_REPEAT_DELAY 300000
    #define RG_DIALOG_REPEAT_MIN    40000
    #define RG_DIALOG_NAV_KEYS     (RG_KEY_UP | RG_KEY_DOWN)

    rg_gui_event_t event = RG_DIALOG_VOID;
    uint32_t joystick = 0;
    uint64_t joystick_last = 0;
    int64_t repeat_delay = RG_DIALOG_REPEAT_DELAY;

    while (event != RG_DIALOG_SELECT && event != RG_DIALOG_CANCEL)
    {
        const uint32_t joystick_prev = joystick;
        joystick = rg_input_read_gamepad();
        event = RG_DIALOG_VOID;

        const bool changed = (joystick != joystick_prev);
        const bool navigating = (joystick & RG_DIALOG_NAV_KEYS) != 0;
        const int64_t held = rg_system_timer() - joystick_last;
        const bool repeat = !changed && joystick &&
                            held > (navigating ? repeat_delay : RG_DIALOG_REPEAT_DELAY);

        if (changed)
            repeat_delay = RG_DIALOG_REPEAT_DELAY;
        else if (repeat && navigating)
            repeat_delay = RG_MAX(RG_DIALOG_REPEAT_MIN, repeat_delay * 2 / 3);
        ...
        if (redraw)
        {
            draw_dialog(title, options, sel, !layout_dirty);
            layout_dirty = false;
            redraw = false;
        }
    ...
    // Drop the cached layout before `options` goes away: it keys on that pointer, and
    // a later allocation landing on the same address must not match a stale entry.
    free(layout_cache.row_height);
    layout_cache = (__typeof__(layout_cache)){0};
    free(options);
    return result;
}
...
static rg_gui_event_t vcom_offset_update_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int v = rg_display_get_vcom_offset();
    if (event == RG_DIALOG_PREV && v > 0)    v--;
    if (event == RG_DIALOG_NEXT && v < 0x3F) v++; // VCMOFS is 6-bit (0x00-0x3F), >0x3F hits reserved bits
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
        rg_display_set_vcom_offset((uint8_t)v);
    sprintf(option->value, "0x%02X", v);
    return RG_DIALOG_VOID;
}

static rg_gui_event_t vrh_update_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int v = rg_display_get_vrh();
    if (event == RG_DIALOG_PREV && v > 0)    v--;
    if (event == RG_DIALOG_NEXT && v < 0x27) v++; // Datasheet: VRHS above 0x27 is reserved
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
        rg_display_set_vrh((uint8_t)v);
    sprintf(option->value, "0x%02X", v);
    return RG_DIALOG_VOID;
}

static rg_gui_event_t framerate_update_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int v = rg_display_get_framerate();
    if (event == RG_DIALOG_PREV && v > 0)    v--;
    if (event == RG_DIALOG_NEXT && v < 0x1F) v++;
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
        rg_display_set_framerate((uint8_t)v);
    sprintf(option->value, "0x%02X", v); // 0x00=119Hz, 0x0F=60Hz; lower value = higher Hz
    return RG_DIALOG_VOID;
}
...
static rg_gui_event_t color_reset_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_ENTER) {
        rg_display_set_vcom(0x34);
        rg_display_set_vcom_offset(0x20);
        rg_display_set_vrh(0x0F);
        rg_display_set_framerate(0x02);
...
// Live crosstalk tuning screen. Shows a full white field with a centered black
// block (the worst case for column crosstalk: the faint off-white/grey streaks
// beside dark objects) and lets the four hardware knobs that affect it be adjusted
// while the pattern stays fully visible. The normal menu can't do this because its
// dialog box covers the screen.
//   U/D = pick register, L/R = adjust by 1, Start = next pattern, A = reset, B = exit.
static rg_gui_event_t crosstalk_test_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event != RG_DIALOG_ENTER)
        return RG_DIALOG_VOID;

    const int W = gui.screen_width;
    const int H = gui.screen_height;
    const int bw = W / 3;
    const int bh = H / 3;

    int sel = 0;     // 0 = VCOM Offset (0xC5), 1 = VRH (0xC3), 2 = VCOM (0xBB), 3 = Framerate (0xC6)
    int pattern = 0; // 0 = black block, 1 = white block, 2 = L/R split, 3 = T/B split, 4 = flat white, 5 = flat grey
    bool redraw_text = true;
    bool redraw_pattern = true;
    ...
    while (1)
    {
        if (redraw_pattern)
        {
            switch (pattern)
            {
            case 0: // black block on white
                rg_gui_draw_rect(0, 0, W, H, 0, 0, C_WHITE);
                rg_gui_draw_rect((W - bw) / 2, (H - bh) / 2, bw, bh, 0, 0, C_BLACK);
                break;
            case 1: // white block on black (inverse polarity)
                rg_gui_draw_rect(0, 0, W, H, 0, 0, C_BLACK);
                rg_gui_draw_rect((W - bw) / 2, (H - bh) / 2, bw, bh, 0, 0, C_WHITE);
                break;
            case 2: // left black / right white (one long vertical edge)
                rg_gui_draw_rect(0, 0, W / 2, H, 0, 0, C_BLACK);
                rg_gui_draw_rect(W / 2, 0, W - W / 2, H, 0, 0, C_WHITE);
                break;
            case 3: // top black / bottom white (one long horizontal edge)
                rg_gui_draw_rect(0, 0, W, H / 2, 0, 0, C_BLACK);
                rg_gui_draw_rect(0, H / 2, W, H - H / 2, 0, 0, C_WHITE);
                break;
            case 4: // flat white field, no block
                rg_gui_draw_rect(0, 0, W, H, 0, 0, C_WHITE);
                break;
            default: // flat mid-grey field (panel uniformity / banding)
                rg_gui_draw_rect(0, 0, W, H, 0, 0, C_RGB(128, 128, 128));
                break;
            }
        ...
        {
            char line[64];
            snprintf(line, sizeof(line), "%c VCOM Offset 0xC5 = 0x%02X", sel == 0 ? '>' : ' ', rg_display_get_vcom_offset());
            rg_gui_draw_text(2, 2,  W - 4, line, sel == 0 ? C_BLACK : C_DIM_GRAY, C_WHITE, 0);
            snprintf(line, sizeof(line), "%c VRH        0xC3 = 0x%02X", sel == 1 ? '>' : ' ', rg_display_get_vrh());
            rg_gui_draw_text(2, 16, W - 4, line, sel == 1 ? C_BLACK : C_DIM_GRAY, C_WHITE, 0);
            snprintf(line, sizeof(line), "%c VCOM       0xBB = 0x%02X", sel == 2 ? '>' : ' ', rg_display_get_vcom());
            rg_gui_draw_text(2, 30, W - 4, line, sel == 2 ? C_BLACK : C_DIM_GRAY, C_WHITE, 0);
            snprintf(line, sizeof(line), "%c Framerate  0xC6 = 0x%02X (00=119Hz 0F=60Hz)", sel == 3 ? '>' : ' ', rg_display_get_framerate());
            rg_gui_draw_text(2, 44, W - 4, line, sel == 3 ? C_BLACK : C_DIM_GRAY, C_WHITE, 0);
            rg_gui_draw_text(2, H - 14, W - 4, "U/D:reg  L/R:adj  Start:pattern  A:reset  B:exit", C_DIM_GRAY, C_WHITE, 0);
            redraw_text = false;
        }

        uint32_t joystick = rg_input_read_gamepad();

        if (joystick & (RG_KEY_B | RG_KEY_MENU | RG_KEY_OPTION))
            break;
        else if (joystick & RG_KEY_START)
            { pattern = (pattern + 1) % 6; redraw_pattern = true; }
        else if (joystick & RG_KEY_UP)
            { sel = (sel + 3) % 4; redraw_text = true; }
        else if (joystick & RG_KEY_DOWN)
            { sel = (sel + 1) % 4; redraw_text = true; }
        else if (joystick & (RG_KEY_LEFT | RG_KEY_RIGHT))
        {
            int dir = (joystick & RG_KEY_RIGHT) ? 1 : -1;
            if (sel == 0)
            {
                int v = rg_display_get_vcom_offset() + dir;
                rg_display_set_vcom_offset((uint8_t)(v < 0 ? 0 : v > 0x3F ? 0x3F : v));
            }
            else if (sel == 1)
            {
                int v = rg_display_get_vrh() + dir;
                rg_display_set_vrh((uint8_t)(v < 0 ? 0 : v > 0x27 ? 0x27 : v));
            }
            else if (sel == 2)
            {
                int v = rg_display_get_vcom() + dir;
                rg_display_set_vcom((uint8_t)(v < 0 ? 0 : v > 0x3F ? 0x3F : v));
            }
            else
            {
                int v = rg_display_get_framerate() + dir;
                rg_display_set_framerate((uint8_t)(v < 0 ? 0 : v > 0x1F ? 0x1F : v));
            }
            redraw_text = true;
        }
        else if (joystick & RG_KEY_A)
        ...
        if (joystick)
            rg_input_wait_for_key(joystick, false, 1000); // debounce until release
        rg_task_delay(20);
    }
...
static rg_gui_event_t color_menu_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_ENTER) {
        const rg_gui_option_t color_options[] = {
            ...
            {0, _("VCOM Offset"), "-",  RG_DIALOG_FLAG_NORMAL, &vcom_offset_update_cb},
            {0, _("VRH"),         "-",  RG_DIALOG_FLAG_NORMAL, &vrh_update_cb},
            {0, _("Framerate"),   "-",  RG_DIALOG_FLAG_NORMAL, &framerate_update_cb},
            {0, _("Crosstalk test"), NULL, RG_DIALOG_FLAG_NORMAL, &crosstalk_test_cb},
...
static rg_gui_event_t audio_update_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    if (sink != prev_sink)
    {
        const rg_audio_sink_t *target = &sinks[sink];

#ifdef RG_AUDIO_NO_HOTPLUG_SINK
        // This app hasn't the internal RAM to host the USB stack (see
        // components/retro-go/CMakeLists.txt). Say so instead of accepting a
        // choice that would go silent or drop out after a few seconds.
        if (target->driver->device_ready)
        {
            rg_gui_alert(_("USB-C DAC"), _("Not available on GBA"));
            sink = prev_sink;
            strcpy(option->value, sinks[sink].name);
            return RG_DIALOG_REDRAW; // the alert painted over the menu
        }
#endif

        // Every sink switches live, including hot-pluggable ones (USB-C DAC).
        // Those bring up a 5V VBUS rail whose inrush can brown out the device;
        // if that happens we reset, and the brown-out guard in rg_audio_init
        // reverts to the speaker on the next boot, so it can't get stuck. The
        // sink is also brought up before the device has enumerated -- the UAC
        // driver's vbus task waits for it -- so audio stays silent until the
        // DAC is actually plugged in. That's what rg_audio_sink_ready() is for.
        rg_audio_set_sink(target->driver->name, target->device);
    }
...
#ifdef RG_GPIO_LED
// Cycles Auto (brightness follows the battery) -> On -> Off. Saved in NVS.
static rg_gui_event_t led_mode_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    int mode = rg_system_get_led_mode();
    if (event == RG_DIALOG_NEXT)
        rg_system_set_led_mode((mode + 1) % 3);
    else if (event == RG_DIALOG_PREV)
        rg_system_set_led_mode((mode + 2) % 3);
    mode = rg_system_get_led_mode();
    strcpy(option->value, mode == RG_LED_MODE_AUTO ? _("Auto") : mode == RG_LED_MODE_ON ? _("On") : _("Off"));
    return RG_DIALOG_VOID;
}

// Blink the status LED below the warning voltage. Off by default; ignored while
// the LED is switched off entirely.
static rg_gui_event_t led_lowbat_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
        rg_system_set_led_lowbat_blink(!rg_system_get_led_lowbat_blink());
    strcpy(option->value, rg_system_get_led_lowbat_blink() ? _("On") : _("Off"));
    return RG_DIALOG_VOID;
}
#endif

// The second LED (RG_GPIO_LED2) has no menu entry yet; it is parked off at boot.
// rg_system_set_led2()/rg_system_get_led2() are still there to drive it once we
// decide what it should indicate.
static rg_gui_event_t led_indicator_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_ENTER)
    {
        const rg_gui_option_t options[] = {
            #ifdef RG_GPIO_LED
            {0, _("Status LED"), "-", RG_DIALOG_FLAG_NORMAL, &led_mode_cb},
            {0, _("Low batt. blink"), "-", RG_DIALOG_FLAG_NORMAL, &led_lowbat_cb},
            #endif
            RG_DIALOG_END,
        };
...
// The result is committed to the system clock + RTC once, when the dialog closes.
...
static rg_gui_event_t set_clock_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    ...
    if (t > 0)
    {
        ...
        rg_system_save_time(); // persist to RTC + storage once, on exit
    }
    rg_settings_commit();
    return RG_DIALOG_REDRAW;
}
...
// to off for security: it has no authentication).
...
// On-screen low-battery icon. Off by default because the status LED blinks for
// the same condition; boards without that LED want it on.
static rg_gui_event_t battery_icon_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    bool show = rg_display_get_battery_icon();
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
    {
        show = !show;
        rg_display_set_battery_icon(show);
    }
    strcpy(option->value, show ? _("On") : _("Off"));
    return RG_DIALOG_VOID;
}
...
void rg_gui_options_menu(void)
{
    ...
    if (app_check->isLauncher)
    {
        ...
        // Hidden on V3: this board already blinks the status LED on a low
        // battery, so the on-screen icon is redundant. Still reachable from
        // the service menu.
        entries[n++] = (rg_gui_menu_entry_t){ 1, "BattIcon",   false, {0, _("Batt. icon"),     "-", RG_DIALOG_FLAG_NORMAL, &battery_icon_cb} };
        ...
        #ifdef RG_GPIO_LED
        entries[n++] = (rg_gui_menu_entry_t){ 0, "LEDOptions", false, {0, _("LED options"),    NULL, RG_DIALOG_FLAG_NORMAL, &led_indicator_cb} };
        #endif
        ...
        // -------- Launcher handler fills tab 0 and tab 1 --------
        ...
        // -------- SYSTEM (tab 1) -- management (at the bottom) --------
    ...
    // Service-only entries (Color settings, Screen position, LED): hidden by default,
    ...
    int led_flag    = rg_gui_get_option_visible("LEDOptions", false) ? RG_DIALOG_FLAG_NORMAL : RG_DIALOG_FLAG_HIDDEN;
...
#endif
        #ifdef RG_GPIO_LED
        {0, _("LED options"),   NULL, led_flag, &led_indicator_cb},
        #endif
        #ifdef RG_ENABLE_NETWORKING
...
static rg_gui_event_t about_popup_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_ENTER)
    {
        ...
        // last one, shifting the view to the second "page" with the rest.
        // The DMG theme renders both colours identically (0x2A83), Classic a
        // little lighter for MESSAGE -- both readable.
        // V2 and V3 share RG_TARGET_NAME, so show what the hardware marker says
        // this board actually is -- useful for support questions. Keep the
        // string no wider than RG_TARGET_NAME: a longer value grows col2_width,
        // which is then clamped to the box after the row heights have already
        // been measured, so the value wraps onto a second line and the page
        // boundaries used for scrolling no longer match what gets drawn.
        static char target_line[48];
#ifdef RG_HW_MARKER_MAGIC
        snprintf(target_line, sizeof(target_line), "%s %s", RG_PROJECT_NAME,
                 rg_system_hw_marker_matches() ? "V3" : "V2?");
#else
        snprintf(target_line, sizeof(target_line), "%s", RG_TARGET_NAME);
#endif

        const rg_gui_option_t options[] = {
            {0, "Version",   (char *)RG_PROJECT_VER,                       RG_DIALOG_FLAG_DISABLED, NULL},
            {0, "Target",    target_line,                                  RG_DIALOG_FLAG_MESSAGE,  NULL},
