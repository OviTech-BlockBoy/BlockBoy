...
typedef struct
{
    ...
    void (*tabbed_options)(rg_gui_menu_entry_t *dest);              // Fills entries for the launcher's Launcher tab
} rg_handlers_t;
...
// Status LED behaviour (persisted in NVS, applied again at boot).
#define RG_LED_MODE_AUTO 0 // brightness follows the battery level (default)
#define RG_LED_MODE_ON   1 // always lit at full brightness
#define RG_LED_MODE_OFF  2 // always dark
void rg_system_set_led_mode(int mode);
int  rg_system_get_led_mode(void);
// Blink the status LED when the battery drops below the warning voltage.
// Persisted in NVS, off by default. Ignored while the mode is RG_LED_MODE_OFF.
...
// Showcase in-app switching: an app that can change games without rebooting (e.g. the
// GB/GBC emulator) calls rg_system_set_inapp_showcase(true), then polls
// rg_system_showcase_next_pending() in its game loop to load the next ROM in-place.
...
// Wi-Fi power mode (showcase / low-battery internet time)
typedef enum {
    RG_WIFI_MODE_OFF       = 0,
    RG_WIFI_MODE_ON        = 1,
    RG_WIFI_MODE_SYNC_ONLY = 2,
} rg_wifi_mode_t;

int  rg_system_get_wifi_mode(void);
void rg_system_set_wifi_mode(int mode);
// Briefly enables Wi-Fi, waits for SNTP to set the clock, turns Wi-Fi off. Blocking (~3-15s).
