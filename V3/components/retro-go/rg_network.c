...
#ifdef ESP_PLATFORM
// Publishes 'blockboy.local' on the LAN so the file server is reachable by name
// instead of a DHCP address that can change. Only started in the launcher (where
// the server runs); emulators skip it to save internal RAM.
static bool mdns_started;
static void start_mdns(void)
{
    if (mdns_started || !rg_system_get_app()->isLauncher)
        return;
    if (mdns_init() != ESP_OK)
        return;
    mdns_hostname_set("blockboy");
    mdns_instance_name_set("BlockBoy");
    mdns_service_add(NULL, "_http", "_tcp", 80, NULL, 0);
    mdns_started = true;
    RG_LOGI("mDNS started: http://blockboy.local/\n");
}
...
bool rg_network_wifi_delete_config(int slot)
{
    ...
    rg_settings_delete(NS_WIFI, key_ssid);
    rg_settings_delete(NS_WIFI, key_password);
    rg_settings_delete(NS_WIFI, key_channel);
    rg_settings_delete(NS_WIFI, key_mode);
    return true;
}
...
bool rg_network_wifi_start(void)
{
    ...
    else
    {
        ...
        TRY(esp_wifi_start());
        // Low-power tuning (best-effort, non-fatal): let the radio modem-sleep
        // between beacons, and cut TX power from the ~20 dBm default to ~11 dBm.
        // The big TX current spikes are what briefly dim the backlight; this
        // keeps background Wi-Fi light and reduces the dimming.
        esp_wifi_set_ps(WIFI_PS_MIN_MODEM);
        esp_wifi_set_max_tx_power(44); // 0.25 dBm units -> 11 dBm
    }
    return true;
fail:
#endif
...
int rg_network_wifi_scan(rg_wifi_scan_result_t *results, int max_results)
{
#ifdef RG_ENABLE_NETWORKING
    ...
    {
        RG_LOGW("Wi-Fi scan requested but network not initialised");
        return -1;
    }

    esp_err_t err;

    // If Wi-Fi is already up (connected or connecting) we scan in place: a
    // foreground scan briefly hops channels and returns to the AP, so the
    // connection is kept. Tearing the connection down here used to make
    // esp_wifi_scan_start fail with a state error ("Scan failed") whenever you
    // were already on a network. Only bring the STA up ourselves when Wi-Fi
    // isn't started yet.
    if (network_state != RG_NETWORK_CONNECTED && network_state != RG_NETWORK_CONNECTING)
    {
        esp_wifi_stop();
        err = esp_wifi_set_mode(WIFI_MODE_STA);
        if (err != ESP_OK) { RG_LOGE("set_mode(STA) failed: 0x%x", err); return -1; }
        err = esp_wifi_start();
        if (err != ESP_OK) { RG_LOGE("wifi_start failed: 0x%x", err); return -1; }
    }

    wifi_scan_config_t scan_cfg = {
        .ssid = NULL, .bssid = NULL, .channel = 0, .show_hidden = false,
        .scan_type = WIFI_SCAN_TYPE_ACTIVE,
        .scan_time.active.min = 120, .scan_time.active.max = 300,
    };

    RG_LOGI("Scanning for Wi-Fi networks...");
    err = esp_wifi_scan_start(&scan_cfg, true); // blocking
    if (err != ESP_OK) { RG_LOGE("scan_start failed: 0x%x", err); return -1; }

    uint16_t ap_count = 0;
    esp_wifi_scan_get_ap_num(&ap_count);
    if (ap_count == 0) { RG_LOGI("No Wi-Fi networks found"); return 0; }

    uint16_t fetch = ap_count;
    wifi_ap_record_t *records = calloc(fetch, sizeof(wifi_ap_record_t));
    if (!records) { RG_LOGE("OOM scan buffer"); return -1; }
    err = esp_wifi_scan_get_ap_records(&fetch, records);
    if (err != ESP_OK) { RG_LOGE("get_ap_records failed: 0x%x", err); free(records); return -1; }

    int out_count = fetch < (uint16_t)max_results ? fetch : max_results;
    for (int i = 0; i < out_count; ++i)
    {
        memccpy(results[i].ssid, records[i].ssid, 0, 32);
        results[i].ssid[32] = '\0';
        results[i].rssi = records[i].rssi;
        results[i].channel = records[i].primary;
        results[i].auth_mode = (int)records[i].authmode;
    }
    free(records);

    RG_LOGI("Wi-Fi scan found %d networks (returning %d)", ap_count, out_count);
    return out_count;
#else
    return -1;
#endif
...
rg_http_req_t *rg_network_http_open(const char *url, const rg_http_cfg_t *cfg)
{
    ...
    #ifdef CONFIG_MBEDTLS_CERTIFICATE_BUNDLE
        // Without a CA bundle esp-tls accepts any certificate (see
        // CONFIG_ESP_TLS_SKIP_SERVER_CERT_VERIFY). For images that is
        // tolerable, but not for fetching firmware over such a connection:
        // anyone on the same network could replace both the manifest and the
        // binaries, and the checksums come from that same manifest.
