...
#define UAC_VBUS_STACK             3072
#define UAC_VBUS_ATTEMPTS          4     // power-cycle VBUS up to this many times to force a clean attach
#define UAC_VBUS_WAIT_MS           1500  // how long to wait for enumeration after each VBUS-on
#define UAC_VBUS_OFF_MS            250   // VBUS off-time between retries
...
static struct {
    ...
    TaskHandle_t vbus_task;     // drives RG_GPIO_BOOST_EN: enables VBUS + power-cycle retry
    volatile bool vbus_run;     // cleared by driver_deinit to stop the vbus task
...
// Rates the USB audio class devices actually implement. Anything else gets
// converted by the resampler in submit() instead of being negotiated.
static bool uac_rate_is_standard(uint32_t hz)
{
    return hz == 8000 || hz == 16000 || hz == 32000 || hz == 44100 || hz == 48000;
}
...
static void uac_worker_task(void *arg)
{
    ...
    while (xQueueReceive(state.worker_queue, &msg, portMAX_DELAY) == pdTRUE)
    {
        ...
        // Ask for a rate the USB audio class actually standardises. Emulators use
        // rates no dongle implements (GBA: 16384 Hz); submit()'s resampler bridges
        // the difference. Trying the odd rate first wastes a negotiation and can
        // leave the interface half-configured, so the retry fails too.
        uint32_t want_rate = state.sample_rate;
        if (!uac_rate_is_standard(want_rate))
        {
            RG_LOGI("UAC worker: %d Hz is not a standard rate, asking for %d Hz\n",
                    state.sample_rate, UAC_FALLBACK_SAMPLE_RATE);
            want_rate = UAC_FALLBACK_SAMPLE_RATE;
        }
        ...
        // Retry on ANY rejection, not just ESP_ERR_NOT_FOUND. Dongles report an
        // unsupported rate with whatever code they like (INVALID_ARG,
        // NOT_SUPPORTED, ...), and skipping the retry left dev_handle NULL --
        // the game then ran at the right speed but in silence. Hit by GBA,
        // which asks for 16384 Hz; GBC's 32000 Hz is a common rate and passed.
        if (err != ESP_OK && stream_cfg.sample_freq != UAC_FALLBACK_SAMPLE_RATE)
        {
            RG_LOGW("UAC worker: %d Hz rejected (%s), retrying %d Hz\n",
                    state.sample_rate, esp_err_to_name(err), UAC_FALLBACK_SAMPLE_RATE);
...
#ifdef RG_GPIO_BOOST_EN
// Brings up VBUS and makes sure a device actually enumerates. The first power-up
// after boot frequently misses the attach (VBUS/boost still settling), so if no
// device shows up we power-cycle VBUS to force a fresh, clean attach -- this
// automates the manual "tap a cap on the EN line" workaround. Once a device is
// connected (or attempts are exhausted) VBUS is left on so a later hot-plug
// still works. driver_deinit clears state.vbus_run to stop us early.
static void uac_vbus_task(void *arg)
{
    (void)arg;
    for (int attempt = 0; state.vbus_run && attempt < UAC_VBUS_ATTEMPTS; ++attempt)
    {
        gpio_set_level(RG_GPIO_BOOST_EN, 1);

        bool connected = false;
        for (int waited = 0; waited < UAC_VBUS_WAIT_MS && state.vbus_run; waited += 100)
        {
            vTaskDelay(pdMS_TO_TICKS(100));
            xSemaphoreTake(state.dev_mutex, portMAX_DELAY);
            connected = (state.dev_handle != NULL);
            xSemaphoreGive(state.dev_mutex);
            if (connected)
                break;
        }
        ...
        if (state.vbus_run && attempt + 1 < UAC_VBUS_ATTEMPTS)
        {
            RG_LOGW("UAC: no device after VBUS on; power-cycling (attempt %d/%d)\n",
                    attempt + 1, UAC_VBUS_ATTEMPTS);
            gpio_set_level(RG_GPIO_BOOST_EN, 0);
            vTaskDelay(pdMS_TO_TICKS(UAC_VBUS_OFF_MS));
        }
    }

    // If we were told to stop (teardown), leave VBUS off; otherwise leave it on.
    if (!state.vbus_run)
        gpio_set_level(RG_GPIO_BOOST_EN, 0);

    state.vbus_task = NULL;
    vTaskDelete(NULL);
}
#endif
...
#ifdef RG_GPIO_BOOST_EN
    gpio_set_direction(RG_GPIO_BOOST_EN, GPIO_MODE_OUTPUT);
    gpio_set_level(RG_GPIO_BOOST_EN, 0); // vbus task brings it up (and retries)
    state.vbus_run = true;
    if (xTaskCreatePinnedToCore(uac_vbus_task, "uac_vbus", UAC_VBUS_STACK, NULL, 4, &state.vbus_task, 0) != pdPASS)
    {
        state.vbus_run = false;
        state.vbus_task = NULL;
        gpio_set_level(RG_GPIO_BOOST_EN, 1); // fallback: just enable it
        RG_LOGW("UAC: failed to spawn vbus task; VBUS enabled without retry\n");
    }
    RG_LOGI("UAC: VBUS management started (GPIO %d)\n", RG_GPIO_BOOST_EN);
#endif
...
{
#ifdef RG_GPIO_BOOST_EN
    state.vbus_run = false; // ask the vbus task to stop
    for (int i = 0; i < 15 && state.vbus_task; ++i)
        vTaskDelay(pdMS_TO_TICKS(20)); // give it a chance to exit on its own
    if (state.vbus_task)
    {
        vTaskDelete(state.vbus_task);
        state.vbus_task = NULL;
    }
    gpio_set_level(RG_GPIO_BOOST_EN, 0);
#endif
...
// Reports whether a USB-C DAC has actually enumerated. rg_audio uses this to
// tell "sink selected" apart from "sink selected and a dongle is present",
// which drives the menu's connect/timeout fallback to the speaker.
static bool driver_device_ready(void)
{
    return state.dev_handle != NULL;
}
...
const rg_audio_driver_t rg_audio_driver_uac_host = {
    ...
    .device_ready = driver_device_ready,
};
