...
static const char *SETTING_BATTERY_CAL = "BattCalPct";
#define DEFAULT_BATTERY_CAL 100

int rg_input_get_battery_cal(void)
{
    return rg_settings_get_number(NS_GLOBAL, SETTING_BATTERY_CAL, DEFAULT_BATTERY_CAL);
}

void rg_input_set_battery_cal(int pct)
{
    rg_settings_set_number(NS_GLOBAL, SETTING_BATTERY_CAL, RG_MAX(50, RG_MIN(200, pct)));
}
...
static uint32_t gamepad_passthrough = 0;
...
bool rg_input_read_battery_raw(rg_battery_t *out)
{
    ...
    raw_value /= 4;
    // Apply the user-trimmed divider/ADC calibration (menu-adjustable, see
    // rg_input_set_battery_cal) on top of the compile-time RG_BATTERY_VDIV_RATIO.
    raw_value = raw_value * rg_input_get_battery_cal() / 100;
...
#warning "not implemented"
    ...
    // BLE gamepad, if connected (also feeds the virtual map below so
    // Start+Select opens the menu from a wireless controller too)
    state |= rg_bluetooth_read_gamepad();
...
static void input_task(void *arg)
{
    ...
    // Auto-repeat timing for volume/brightness buttons
    int64_t last_action_time = 0;
    uint32_t prev_volume_keys_stable = 0;
    ...
    while (input_task_running)
    {
        if (rg_input_read_gamepad_raw(&state))
        {
            ...
            // Handle hardware volume and brightness buttons (0v1tech-lego-gb)
            // Keys in gamepad_passthrough are NOT intercepted (passed to the app instead, e.g. GBA L/R)
            // Simple approach: trigger on press OR if held for 200ms intervals
            uint32_t hw_keys = (RG_KEY_L | RG_KEY_R | RG_KEY_X | RG_KEY_Y) & ~gamepad_passthrough;
            uint32_t current_volume_keys = local_gamepad_state & hw_keys;
            int64_t now = rg_system_timer();
            bool trigger_action = false;

            // Check if button state changed OR enough time passed while held
            if (current_volume_keys != prev_volume_keys_stable)
            {
                // Button state changed
                if (current_volume_keys != 0)
                {
                    // New button pressed - trigger immediately
                    trigger_action = true;
                    last_action_time = now;
                }
                prev_volume_keys_stable = current_volume_keys;
            }
            else if (current_volume_keys != 0)
            {
                // Same button still held - check if 200ms passed since last action
                if ((now - last_action_time) >= 200000) // 200ms in microseconds
                {
                    trigger_action = true;
                    last_action_time = now;
                }
            ...
            // This task is started by rg_input_init(), which runs EARLIER in
            // rg_system_init() than rg_display_init() and rg_audio_init(). A wheel
            // glitch during the power-on ramp (the ADC ladder sweeps through the
            // X/Y bands while its node charges) would then act on subsystems that
            // don't exist yet: rg_display_get_backlight() still returns 0, the step
            // below computes 0/10, and rg_display_set_backlight() PERSISTS that.
            // rg_display_init() reads the poisoned value back a moment later and
            // brings the panel up at 1% -- the device boots and plays the boot
            // sound but the screen looks dead. Keep the wheels inert until boot is
            // complete. The keys are still masked out of gamepad_state below, so
            // a glitch can't trip the recovery/pairing checks either.
            if (trigger_action && !rg_system_get_app()->initialized)
            {
                trigger_action = false;
                RG_LOGW("Ignoring wheel input during boot (system not initialized yet)\n");
            }

            if (trigger_action)
            {
                if (current_volume_keys & RG_KEY_L) // Volume UP
                {
                    int volume = rg_audio_get_volume();
                    volume = RG_MIN(100, volume + 5);
                    volume -= (volume % 5); // Round to nearest 5%
                    rg_audio_set_volume(volume);
                }
                if (current_volume_keys & RG_KEY_R) // Volume DOWN
                {
                    int volume = rg_audio_get_volume();
                    volume = RG_MAX(0, volume - 5);
                    volume -= (volume % 5); // Round to nearest 5%
                    rg_audio_set_volume(volume);
                }
                if (current_volume_keys & RG_KEY_X) // Brightness UP
                {
                    int brightness = rg_display_get_backlight();
                    brightness = RG_MIN(100, brightness + 10);
                    brightness -= (brightness % 10); // Round to nearest 10%
                    rg_display_set_backlight(brightness);
                }
                if (current_volume_keys & RG_KEY_Y) // Brightness DOWN
                {
                    int brightness = rg_display_get_backlight();
                    // Step down 10% and snap to a multiple of 10, floor at 10%.
                    // The old form was RG_MAX(1, b - 10) followed by b -= b % 10,
                    // so from 10% it produced 1 - 1 = 0 (clamped to 1% by
                    // rg_display_set_backlight): a single notch blanked the screen.
                    brightness = RG_MAX(10, ((brightness + 9) / 10) * 10 - 10);
                    rg_display_set_backlight(brightness);
                }
            }

            local_gamepad_state &= ~hw_keys; // Remove intercepted keys from gamepad state
...
void rg_input_set_gamepad_passthrough(uint32_t keys)
{
    gamepad_passthrough = keys;
}
