...
PROJECT_APPS = {}
PROJECT_FLASHER = None
PROJECT_SPARE_APPS = []
def target_firmware_version(target):
    """RG_FIRMWARE_VERSION straight from the target's config.h.

    This is the version the firmware reports and the OTA manifest compares
    against, so deriving the image name from it keeps the file, the About
    screen and the manifest in step. Falling back to a hardcoded string got us
    a 'blockboy_v2.0.1_...img' sitting in the V3 folder.
    """
    path = os.path.join("components", "retro-go", "targets", target, "config.h")
    try:
        with open(path, "r", encoding="utf-8", errors="replace") as fp:
            found = re.search(r'#\s*define\s+RG_FIRMWARE_VERSION\s+"([^"]+)"', fp.read())
        return found.group(1) if found else None
    except OSError:
        return None


def resolve_project_ver(target):
    """PROJECT_VER wins (explicit override), then the target config, then git."""
    if os.getenv("PROJECT_VER"):
        return os.getenv("PROJECT_VER")
    from_target = target_firmware_version(target)
    if from_target:
        return from_target
    try:
...
PROJECT_VER = "unknown"  # replaced once the target is known, see below
...
def build_image(output_file, apps, img_format="esp32", fatsize=0):
    ...
    for app in apps:
        with open(os.path.join(app, "build", app + ".bin"), "rb") as f:
            data = f.read()
        # The layout is frozen for OTA (see rg_config.py). Never grow it
        # silently: that shifts everything behind it and breaks OTA on devices
        # already with customers.
        part_size = PROJECT_APPS[app][2]
        if len(data) > part_size:
            exit("\nERROR: '%s' is %d bytes too big for its partition "
                 "(%d of %d used).\nThe OTA layout is frozen; growing it breaks "
                 "updates on existing devices.\nShrink the app, or adjust the layout "
                 "and bump RG_LAYOUT_VERSION.\n"
                 % (app, len(data) - part_size, len(data), part_size))
        headroom = part_size - len(data)
        print("  %-12s %7d / %7d bytes (%d%% used, %d KB free)"
              % (app, len(data), part_size, len(data) * 100 // part_size, headroom // 1024))
        if headroom < 64 * 1024:
            print("  > WARNING: less than 64KB of headroom in '%s'!" % app)
        table_csv.append("%s, app, ota_%d, %d, %d" % (app, table_ota, len(image_data), part_size))
        table_ota += 1
        image_data += data + b"\xFF" * (part_size - len(data))

    # Recovery/flash app as the factory partition, always at the end so it
    # never shifts the offsets of the regular apps.
    if PROJECT_FLASHER:
        fl_name, fl_size = PROJECT_FLASHER
        fl_bin = os.path.join(fl_name, "build", fl_name + ".bin")
        if os.path.exists(fl_bin):
            with open(fl_bin, "rb") as f:
                data = f.read()
            if len(data) > fl_size:
                exit("\nERROR: '%s' does not fit in %d bytes (%d needed).\n"
                     % (fl_name, fl_size, len(data)))
            print("  %-12s %7d / %7d bytes (factory)" % (fl_name, len(data), fl_size))
            table_csv.append("%s, app, factory, %d, %d"
                             % (fl_name, len(image_data), fl_size))
            image_data += data + PAD * (fl_size - len(data))
        else:
            print("  > WARNING: '%s' not built, image gets NO recovery app!"
                  % fl_name)

    # Reserved partitions: in the table, but no bytes in the image. This way a
    # future app can be placed via OTA without overhauling the layout (and thus
    # every device in the field), while the .img stays small.
    spare_offset = len(image_data)
    for spare_name, spare_size in PROJECT_SPARE_APPS:
        table_csv.append("%s, app, ota_%d, %d, %d" % (spare_name, table_ota, spare_offset, spare_size))
        print("  %-12s %7s   %7d bytes (gereserveerd)" % (spare_name, "-", spare_size))
        table_ota += 1
        spare_offset += spare_size

    if fatsize:
        # Use "vfs" label, same as MicroPython, in case the storage is to be shared with a MicroPython install
        table_csv.append("vfs, data, fat, %d, %s" % (spare_offset, fatsize))
...
def build_app(app, device_type, with_profiling=False, no_networking=False, is_release=False):
    ...
    # Per-app sdkconfig overrides: <app>/sdkconfig.overrides wins over the target
    # defaults (e.g. gbsp turns Bluetooth off because the GBA buffers claim all SRAM).
    sdkconfig_files = [f"components/retro-go/targets/{device_type}/sdkconfig", os.path.join(app, "sdkconfig.overrides")]
    sdkconfig_files = [os.path.abspath(f) for f in sdkconfig_files if os.path.exists(f)]
    if sdkconfig_files:
        os.putenv("SDKCONFIG_DEFAULTS", ";".join(sdkconfig_files))
    args = [IDF_PY, "app"]
...
PROJECT_VER = resolve_project_ver(args.target)
print(f"Version: {PROJECT_VER} (target {args.target})")
...
try:
    ...
    if command in ["build", "build-fw", "build-img", "release", "run", "profile", "install"]:
        ...
        # The recovery/flash app belongs in every full image.
        if PROJECT_FLASHER and command in ["build-img", "release", "install"]:
            build_app(PROJECT_FLASHER[0], args.target, False, args.no_networking,
                      command == "release")
